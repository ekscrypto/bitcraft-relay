...
if [ "$DO_STOP" -eq 1 ]; then
    echo "== trial: stop ${UNIT} =="
    run_remote "sudo systemctl stop ${UNIT} 2>/dev/null || true"
    exit 0
fi
...
echo "== trial: install systemd unit =="
rsync_file "$HERE/public-mirror-trial.service" "/etc/systemd/system/${UNIT}"
...
run_remote "sudo systemctl enable ${UNIT}"
run_remote "sudo systemctl restart ${UNIT}"

echo "== trial: wait for mirrors (up to 600s) =="
if [ "$APPLY" -eq 1 ]; then
    ok=0
    i=0
    while [ "$i" -lt 120 ]; do
        if ssh "$SSH_TARGET" "curl -sf --max-time 5 ${MIRRORS_URL}" 2>/dev/null | python3 <<'PY'
import json, sys
...
if want <= live:
    ...
    sys.exit(0)
print("waiting:", sorted(want - live), "not live yet; have", len(mirrors), "mirrors")
sys.exit(1)
PY
        then
            ok=1
    ...
    if [ "$ok" -ne 1 ]; then
        echo "trial mirrors not all live after 600s; check: ssh ${SSH_TARGET} journalctl -u ${UNIT} -n 80" >&2
        exit 1
    fi
    ssh "$SSH_TARGET" "curl -s ${MIRRORS_URL}" | python3 -m json.tool 2>/dev/null || true
else
    printf '$ # (would poll %s until bitcraft-live-7/8/9 are live)\n' "$MIRRORS_URL"
fi
